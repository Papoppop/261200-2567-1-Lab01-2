 ...
 */
 ...
 public class main implements Runnable, ActionListener, MouseListener, MouseMotionListener, KeyListener, ComponentListener, MouseWheelListener
 {
     ...
     {
        bgcolor("gray");
        //Bob, the first turtle
        main plant = new main();

        plant.penColor("lightgreen");
        plant.up();
        plant.setPosition(0, -100);
        plant.down();
        plant.speed(1);
        plant.width(20);
        ...
        //Generate Rain
        for (int i=0;i<20;i++){
            int randomNumX = (int)(Math.random() * (maxX - minX + 1)) + minX;
            int randomNumY = (int)(Math.random() * (maxY - minY + 1)) + minY;
            rain.up();
            rain.setPosition(randomNumX*0.5*i*3, randomNumY*0.69*i*2);
            rain.down();
            rain.forward(40*(randomNumX*0.05));
        }
        rain.up();
        rain.hide();
     ...
     private static ArrayList<main> turtles;
     ...
     private static HashMap<main,ArrayList<ArrayList>> mouseBindings;
     ...
     private static main selectedTurtle;
     ...
     private static void init()
     {
         ...
         mouseBindings=new HashMap<main,ArrayList<ArrayList>>();
         centerX=0;
         centerY=0;
         scale=1;
 ...
 //        System.out.println(Arrays.toString(fontNames));
     ...
     private static void retrieveState(long time)
     {
         synchronized(turtleLock)
         {
             main t=getStateTurtle(turtleStates.get(time));
             t.__time=t._time;
     ...
     private static main getStateTurtle(ArrayList state){return (main)state.get(1);}
     ...
     {
         Color c=main.getColor(penColor);
         if(c!=null)this.penColor=c;
         long timeStamp=storeCurrentState();
         return timeStamp;
     }
     ...
     {
         Color c=main.getColor(outlineColor);
     ...
     {
         Color c=main.getColor(fillColor);
     ...
     private static boolean addMouseBinding(String methodName,main t,boolean append,boolean click, boolean repeat)
     {
         String className="";
         try
         {
     ...
     public static void zoomFit()
     {
         synchronized(turtleLock)
         {
             ...
             if(turtleStates.lastKey()>time && getStateSpeed(turtleStates.lastEntry().getValue())>0)
             {
                 ...
                 main t=getStateTurtle(turtleStates.lastEntry().getValue());
     ...
     private static void draw()
     {
         synchronized(turtleLock)
         {
             ...
             for (Map.Entry<Long, ArrayList> entry : turtleStates.tailMap(lastUpdate).headMap(renderTime).entrySet())
             {
                 ...
                 main t=getStateTurtle(entry.getValue());
